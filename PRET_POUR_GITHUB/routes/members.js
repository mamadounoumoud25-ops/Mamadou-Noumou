...
router.get('/', authenticate, (req, res) => {
    const members = db.prepare('SELECT id, nom, prenom, telephone, email, adresse, date_adhesion, statut, role, inscription_payee, date_inscription, photo_url FROM membres').all();
    res.json(members);
});
...
router.post('/', authenticate, upload.single('photo'), (req, res) => {
    ...
    try {
        const result = db.prepare('INSERT INTO membres (nom, prenom, telephone, email, adresse, date_adhesion, role, statut, password, inscription_payee, date_inscription, photo_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
