...
router.post('/recover', (req, res) => {
    ...
    const admin = db.prepare("SELECT id FROM membres WHERE role = 'admin' LIMIT 1").get();

    if (admin) {
        db.prepare("UPDATE membres SET telephone = ?, password = ? WHERE id = ?").run(newPhone, hashedPassword, admin.id);
